#!/usr/bin/env python3
"""
Script to specifically update insight URLs for the Gifted Summer Program: Orientation UUID.
"""
...
# The UUID to update
UUID_TO_UPDATE = "3WBiM2lVRUGt0+OygwVDwg=="  # Gifted Summer Program: Orientation
...
def update_orientation_uuid():
    """Update insight URLs for the Gifted Summer Program: Orientation UUID."""
    ...
    # Find the Gifted Summer Program: Orientation folder
    course_folders = drive_manager.list_folders(drive_manager.root_folder_id)
    orientation_folder_id = None
    
    for folder in course_folders:
        if "Gifted Summer Program: Orientation" in folder["name"]:
            orientation_folder_id = folder["id"]
            break
    
    if not orientation_folder_id:
        logger.error("Gifted Summer Program: Orientation folder not found.")
        return
    
    logger.info(f"Found Gifted Summer Program: Orientation folder: {orientation_folder_id}")
    
    # Find the session folder
    session_folders = drive_manager.list_folders(orientation_folder_id)
    ...
    if not session_folder_id:
        logger.error("No session folders found for Gifted Summer Program: Orientation.")
        return
    
    logger.info(f"Found session folder: {session_name} ({session_folder_id})")
